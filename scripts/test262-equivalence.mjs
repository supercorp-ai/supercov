...
function runtimePrelude(transformed) {
  ...
  const functions = {
    ...
    registerProbeV2: "function(definition){return Object.assign(definition,{clock:{epoch:1,fast:true},hitEpochs:new Uint32Array(definition.pointIds.length),decisionEpochs:definition.decisions.map(function(meta){return new Uint32Array(2*3**Math.min(meta.conditions.length,6));}),pendingDefaults:new Uint32Array(definition.defaultCount||0)});}",
