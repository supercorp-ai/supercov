...
- Code a probe already recorded runs at close to its own speed: a repeat is one inline comparison for statements, default parameters, `||`, `&&` and `??`, optional members and calls, enumeration loops and caught exceptions alike, with no frame object or runtime call per evaluation, and a script's top-level code no longer records every repeat again. A loop over `??`, `||`, optional calls and `for...of` took 0.56–0.59 s under Supercov and now takes 0.14 s (0.01 s without Supercov), with the same coverage. A million-iteration lru-cache loop took 177 s outside a test and 1.6 s inside one; it now takes 0.3 s either way (0.06 s without Supercov), and minimatch's backtracking guard test passes its one-second limit.
...
- A coverage tool the test command runs itself reports what it reports without Supercov, or close to it. Supercov's MC/DC bookkeeping no longer counts as branches, `a || b` keeps the program's own operator, and a block Supercov adds maps to the statement it wraps. On a sample using every probe kind, Jest's and Vitest's reports match their numbers without Supercov exactly; nyc's is one branch short and c8's one branch over, where they read 36–86% of branches before. nyc's report, which was empty because the copy's source map led outside the workspace, shows the project's files.
...
- Neither are example apps under an `examples/` directory at the top of a project or package, nor root-level `*.conf.*` files such as `wdio.conf.js` and `karma.conf.js`. uuid read 32% of lines because of its nine example apps; it reads 52%, all of it uuid's own code.
