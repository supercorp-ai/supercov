...
- Code a probe already recorded runs at close to its own speed: a repeat is one inline comparison, a default parameter's or destructured default's outcome is counted in a typed array instead of a map keyed by its id, and a script's top-level code no longer records every repeat again. A million-iteration lru-cache loop took 177 s outside a test and 1.6 s inside one; it now takes 0.3 s either way (0.06 s without Supercov), and minimatch's backtracking guard test passes its one-second limit.
