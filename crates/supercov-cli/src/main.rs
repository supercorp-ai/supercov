...
use supercov_engine::{
    ...
    coverage_query::{
        ...
        coverage_covers_query, coverage_dimension_query, coverage_file_decisions_query,
        coverage_file_query, coverage_scope_query, coverage_summary_query, coverage_test_query,
...
fn query_index_files() -> ExitCode {
    ...
    let gaps_only = match request.command.as_str() {
        "files" => Some(false),
        "gaps" => Some(true),
        "file-decisions" | "kinds" | "runners" | "summary" | "scope" | "covers" | "test" => None,
        _ => {
