...
const CAPABILITY_PATTERN = /\b(?:hostPath|guestPath|mounts?|snapshot(?:Key|Id|Tag)?|createPool|acquire|container|machine|sandbox)\b|\.(?:exec|execute|launch|spawn)\s*\(/;
const EXCLUDED_IMPORT = /^(?:node:|@playwright\/test$|playwright$|vitest$|@jest\/globals$)/;
function parserPlugins(url) {
    const plugins = [
        "decorators-legacy",
        "sourcePhaseImports",
    ];
...
export function transformCapabilityImports(source, url, wrapperUrl) {
    if (!CAPABILITY_PATTERN.test(source))
    ...
    traverse(ast, {
        ImportDeclaration(path) {
            ...
            for (const imported of path.node.specifiers) {
                if (t.isImportSpecifier(imported) && imported.importKind === "type")
                    continue;
                ...
                declarations.push(t.variableDeclarator(original, t.callExpression(t.cloneNode(wrapperId), [t.cloneNode(raw)])));
            }
    ...
    if (!transformed || !wrapperId)
        return { code: source, transformed: false };
    ast.program.body.unshift(t.importDeclaration([
        t.importSpecifier(t.cloneNode(wrapperId), t.identifier("wrapImportedCapability")),
